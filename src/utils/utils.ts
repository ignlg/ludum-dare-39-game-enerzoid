...
export function ratio(
  w: number = DEFAULT_GAME_WIDTH,
  h: number = DEFAULT_GAME_HEIGHT
): number {
  let ratio: number
  const wRatio = window.innerWidth / w
  const hRatio = window.innerHeight / h
  if (wRatio >= 1 && hRatio >= 1) {
    ratio = Math.min(Math.floor(wRatio), Math.floor(hRatio))
  } else {
